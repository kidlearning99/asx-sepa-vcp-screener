...
try:
  ...
  if(r.candleSignals && r.candleSignals.length > 0){{
    for(var k=0; k<r.candleSignals.length; k++){{
      ...
      candleSigsHTML += \'<div style="background:var(--bg2);padding:9px;border-radius:6px;margin-bottom:6px;border:1px solid var(--border);border-left:3px solid \'+col+\'\">\'+
        \'<div style="display:flex;align-items:center;gap:6px;margin-bottom:2px\"><span style=\"font-size:13px\">\'+ico+\'</span><span style=\"font-size:11px;font-weight:700;color:\'+col+\'\">\'+sig+\'</span></div>\'+
  ...
  return \'<div class=\"dpanel\">\'+chartCol+sepaCol+fundCol+vcpCol+tradeCol+nuanceHTML+\'</div>\';
    ...
    # Pattern looks for function det(r){{ ... return '<div class="dpanel">...</div>';\n}}
    pattern = re.compile(r'function det\(r\)\{\{.*?return \'<div class="dpanel">\'.*?</div>\';\s*\\\'?\}?\}', re.DOTALL)
    
    # Actually, let's just replace from function det(r) to the next significant block.
    # The file is structured as function det(r){{ ... }} then function tog(i){{ ... }}
    pattern = re.compile(r'function det\(r\)\{\{.*?\}\n\nfunction tog\(i\)\{\{', re.DOTALL)
    
    if not pattern.search(content):
        # Try another pattern if the first fails
        pattern = re.compile(r'function det\(r\)\{\{.*?\}\n\nfunction tog\(i\)', re.DOTALL)

    updated_content = pattern.sub(new_det + '\n\nfunction tog(i)', content)

    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(updated_content)
    
    print("SUCCESS: build_dashboard.py updated with Aggressive Nuances and Correct Layout.")
